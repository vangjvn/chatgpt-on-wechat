...
class WechatMessage(ChatMessage):
    def __init__(self, itchat_msg, is_group=False):
        ...
        if itchat_msg["Type"] == TEXT:
            ...
            if is_group:
                if any(note_bot_join_group in itchat_msg["Content"] for note_bot_join_group in
                       notes_bot_join_group):  # 邀请机器人加入群聊
                    logger.warn("机器人加入群聊消息，不处理~")
                    pass
                elif any(note_join_group in itchat_msg["Content"] for note_join_group in
                    ...
                    if "加入群聊" not in itchat_msg["Content"]:
                        ...
                        if "invited" in itchat_msg["Content"]:  # 匹配英文信息
                            ...
                            re.findall(r'invited\s+(.+?)\s+to\s+the\s+group\s+chat', itchat_msg["Content"])[0]
                        elif "joined" in itchat_msg["Content"]:  # 匹配通过二维码加入的英文信息
                            self.actual_user_nickname = \
                            re.findall(r'"(.*?)" joined the group chat via the QR Code shared by',
                                       itchat_msg["Content"])[0]
                ...
                elif any(note_exit_group in itchat_msg["Content"] for note_exit_group in
                ...
                self.content = itchat_msg["Content"]
            elif any(note_patpat in itchat_msg["Content"] for note_patpat in
                     notes_patpat):  # 若有任何在notes_patpat列表中的字符串出现在NOTE中:
                ...
                if is_group:
                    if "拍了拍我" in itchat_msg["Content"]:  # 识别中文
                        ...
                        self.actual_user_nickname = \
                        re.findall(r'^(.*?)(?:tickled my|tickled me)', itchat_msg["Content"])[0]
            else:
        ...
        # 如果是VIDEO
            ...
            raise NotImplementedError(
                "Unsupported message type: Type:{} MsgType:{}".format(itchat_msg["Type"], itchat_msg["MsgType"]))
